...
uint8_t* next_input(uint8_t*); // return a array of inputs, the pointer value is set to the size of the array
...
// if IMPL defined, define all variables with default init in INIT( . ) if needed, else just declare the variables
#ifndef IMPL
#define QUAL extern
#define INIT(a)
#else
...
#define LOG(FMT, ...) {fprintf(stderr, "%lu : " FMT "\n", millis_since_launch, ##__VA_ARGS__);}
