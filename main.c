...
int main(){
  ...
  for(uint8_t i=0; i<map.height; i++) for(uint8_t j=0; j<map.height; j++){
    map.terrain[i][j]=(terrain_t){.roof= (i+j)&0b1, .floor=((i+j>19)?2:5), .obstacle=' '};
  }
  for(uint8_t i=0; i<map.height; i++){
    map.terrain[i][0]=(terrain_t){.roof= 0b1, .floor=5, .obstacle='#'};
    map.terrain[i][map.height-1]=(terrain_t){.roof= 0b1, .floor=2, .obstacle='#'};
  }
  for(uint8_t j=0; j<map.height; j++){
    map.terrain[0][j]=(terrain_t){.roof= 0b1, .floor=5, .obstacle='#'};
    map.terrain[map.width-1][j]=(terrain_t){.roof= 0b1, .floor=2, .obstacle='#'};
  }


  *entities = (entity_t){.hp=100, .stamina=100, .x=5, .y=5, .type='@'};

  set_ui_input_queue(&input_queue); // segfault if not set before init bc of the input_listener, so to stay uniform every variables are set here at first
  set_ui_map(&map);
  ...
  fprintf(stderr, "quited after %lums, at frame %lu\narverage fps : %3.2f\n", millis_since_launch, frame, (float)millis_since_launch/(1000.*(float) frame));
