...
uint8_t run_frame(void){
  ...
  if(inputs){
    DEBUG(
        ...
        for(uint8_t i=0; i<inputs_c;i++) fprintf(stderr, "%hhu ", inputs[i]);
        fprintf(stderr, "\n");
        free(inputs);
    );

    for(uint8_t i=0; i<inputs_c;i++) switch(inputs[i]){
      case CORE_NONE  : break;

      case CORE_UP    :
        if(entities && map.terrain[entities->y-1][entities->x].obstacle==' '){
          entities->y--;
        }
        break;
      ...
      case CORE_LEFT  :
        if(entities && map.terrain[entities->y][entities->x-1].obstacle==' '){
          entities->x--;
        }
        break;
