...
/*
 * QUAL is extern when this is used as a header, and nothing when this is compiled for implementing the vrariables
 * INIT is used to initialize the variables as its args when IMPL is defined
 */
...
QUAL struct{const entity_t* data; uint16_t count;} entities INIT({});
...
#define LOG(FMT, ...) {fprintf(stderr, "%4lu (ui) : " FMT "\n", frame_count, ##__VA_ARGS__);}
