...
#define COLOR_X_LIST() X(BLACK) X(RED) X(GREEN) X(YELLOW) X(BLUE) X(MAGENTA) X(CYAN) X(WHITE)
...
//colors goes from 0-7 -> w/ background and no color duplicates 64 colors pairs (NOTE: no bright backgournd)
// attributes : 0b 00 000 000
//
// texture plan:
//  - map floor to backround color
//  - map obstacle to char // use plain char but it could heave texture conflict? and could be seem arbitrarry values OR
...
//
//
//  - MAYBE NEED TO MAP PACKED ATTRIBUTES TO THE TERRAIN INSTEAD OF LETTING THAT TO THE API
//
...
int map_texture_to_terrain(const terrain_t* t){
  int attributes={};
  if(!t->roof) attributes|=A_BOLD;

  attributes |= COLOR_PAIR(((t->floor&0b111)<<3)|COLOR_WHITE);// print in white with a background
