...
void close_ui(void){
  ...
  if(listening_inputs) listening_inputs=0;
...
void refresh_ui(void){
  ...
  keypad(win, true);
  if(!active_map){ fprintf(stderr, "no map ;-;\n"); return;} // nothing to draw
  ...
  if(entities.data){
    for(const entity_t* e=entities.data; e < entities.data + entities.count; e++){
      wattrset(win, A_UNDERLINE | COLOR_PAIR(((active_map->terrain[e->y][e->x].floor&0b111)<<3)|COLOR_WHITE));
...
const char* get_ui_info(void){
  return "devel test, TUI build on ncurses\n";
}

void set_ui_map(const map_chunk_t* p_map_chunk){
  active_map = p_map_chunk;
}
